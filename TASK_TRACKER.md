...
**Credentials cannot reach an error string, and cannot reach any `Result` field this adapter
derives. `Result.Title` and `Result.Magnet` are a disclosed exception.** A Torznab request
carries the api_key in its *query string*, so every URL this adapter handles is
credential-bearing, and `internal/logging` masks by key name — which means an opaque key under an
unremarkable name is written out in plaintext. The rule here is therefore structural: **no text
the source sent appears in any error**. That costs the server's own error description, the
`encoding/xml` message (the line number is kept), and the name of an unexpected root element
(classified, not quoted); it buys an adapter with nothing to redact. On the `Result` side,
`TorrentURL` and `SourceURL` legitimately hold credential-bearing URLs and are safe because both
names are on `internal/logging`'s list, and every field the adapter *derives* is kept clean — `ID`
prefers the infohash and strips the query and fragment off any URL-shaped fallback, `Uploader`
refuses a link-shaped value, and `Extra` takes only whitelisted attributes whose value parses as a
number. **`Title` and `Magnet` are neither**: both are the source's own text verbatim, because the
title is the display value and the magnet has to reach the engine as published, and neither name
is masked — so a server that echoes the user's key into a `<title>` or a magnet's `dn=` puts it in
the log file the moment anything logs the `Result`. QA reproduced exactly that on PR #12 against
the real `internal/logging` sink, and an earlier version of this row and of the package doc
claimed it could not happen. `Result.ID` inherits the same gap on the one branch where it falls
back to the title (an item with no infohash, no guid, no comments and no link). The gap belongs to
the frozen §5 `Result` type rather than to this adapter and cannot be closed from here; it is
recorded as backlog `T-934` and reasoned out in DEC-071.
...
against a feed that echoes the key into the title, a magnet's `dn=`, the guid, the comments, the
link, the enclosure, the uploader and the grabs: `TorrentURL`, `SourceURL`, `Title` and `Magnet`
are asserted to carry it, everything derived to be free of it, and a second item whose only
identity is a credential-bearing guid keeps the URL-stripping path exercised.
`TestResultIDFallsBackToTheTitleAndInheritsItsGap` pins the ID branch.
`TestNoCredentialReachesTheLogFile` puts every error plus a parsed `Result` through the real
`internal/logging` sink and greps the file — scoped, as its own comment now says, to the surface
this adapter controls, since a `Title` or `Magnet` carrying a key does reach that file and only
`T-934` can change it. This sweep **caught a live leak while it was
