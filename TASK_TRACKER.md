...
of local-only-green claim this project's QA has bounced other tasks for. **Branch protection on
`main` was not touched**: the three `required_status_checks.contexts` from T-001/DEC-021 are
unchanged, so `indexer-hostnames` reports as an ordinary (non-required) PR check for now, the same
as `build-all` and `licenses` already do — flagging this rather than silently leaving it unstated,
since AGENT.md doesn't ask this task to change branch protection and QA review is what actually
gates the merge per AGENT.md §10 either way.
