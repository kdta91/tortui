...
only hand-maintained expected values left. So adding a bucket fails the build whether or not a
...
test file.) The first version of this test, in PR #9 as originally pushed, compared two lists
that both lived in the test file and so did **not** trip on an added bucket; QA caught that, and
DEC-046 records the false claim and its correction. See DEC-046 and DEC-050.
