...
in the package declaring `CategoryFeatureFilms Category = 7` as the *second* spec of a mixed const
...
`golangci-lint run` → `0 issues.`, and `CategoryFromString("featurefilms")` returning value 7), and
then re-run against the fix, where it is RED. The same is true of the two related shapes QA
reported: a `Category` const in a mixed block with no `init`, and `var CategoryFeatureFilms
Category = 7`. Three further shapes not specifically coded for were probed too: a function-local
`const ... Category = 7` — RED; a multi-name spec `const a, CategoryFeatureFilms Category = 6, 7` —
RED; and `const CategoryFeatureFilms = Category(7)` (untyped, conversion value) — **still green**,
...
probe reverted; `git status` clean and `category.go`'s checksum restored after each. See DEC-051.
