...
re-running `make licenses` twice back-to-back and diffing the output (byte-identical). CI gets a
new `licenses` job (ubuntu-latest only, like `build-all` — the Makefile target already loops all
three GOOS internally, so one host suffices): installs `go-licenses`, runs `make licenses` (which
itself fails the job on a disallowed license), then `git diff --exit-code -- NOTICE` to fail on a
stale file. README's License section already read "MIT. See `LICENSE`." before this task (T-001
wrote it ahead of the file existing) and needed no correction; added one sentence pointing at the
new `NOTICE` file. `make check`, `go test -race ./... -count=1`, and `make licenses` all verified
green locally before opening the PR.
