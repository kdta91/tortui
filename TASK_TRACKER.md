...
omits.** Reported here rather than added quietly: `mmsg` is reached only on the **cgo-enabled**
darwin path via `github.com/anacrolix/go-libutp` (itself MIT), so it appears when a developer runs
`make licenses` natively on a Mac and does *not* appear in any cross-compiled (`CGO_ENABLED=0`)
run — which is why T-031's evidence missed it and why CI on ubuntu would never have surfaced it.
Symmetrically, `anacrolix/utp` appears on the linux/windows/no-cgo paths and not on the native
darwin one. Both must be listed or `make licenses` fails on one host or the other. `mmsg` is
...
The module now has tags: `v0.2.0` carries the upstream MPL-2.0 `LICENSE`, builds against
`anacrolix/torrent v1.61.0`, and leaves **zero `Unknown` rows** in any per-GOOS report, so it is
admitted as a named MPL-2.0 module like the other nine. **T-031 must pin it at `v0.2.0` or later**
— `go get github.com/anacrolix/torrent` on its own selects the unlicensed pseudo-version and will
fail `make licenses`.
...
- `T-031` (2026-09-17). **The MPL-2.0 exception authorised by DEC-098 is narrower than
  ...
  **T-943 landed the policy change (2026-09-18, DEC-100) — two carry-overs for T-031.** (i) The
  ...
  in the tree, reached on the **cgo-enabled** darwin path via `github.com/anacrolix/go-libutp`
  (MIT). It is absent from every cross-compiled (`CGO_ENABLED=0`) report, which is why the evidence
  above missed it; symmetrically `anacrolix/utp` is absent from a native darwin run. Both are in
  the `Makefile`'s `ALLOWED_MPL_MODULES`. (ii) Finding (2) is resolved by **pinning
  `github.com/go-llsqlite/adapter` at `v0.2.0` or later** — that tag carries the upstream MPL-2.0
  `LICENSE` the 2023 pseudo-version lacks, builds against `anacrolix/torrent v1.61.0`, and leaves
