...
- [x] On darwin, startup raises the soft FD limit to the hard limit and `doctor` reports both
  the original and the raised value (AGENT.md §13) — `platform.RaiseFDLimit`. Linux gets the
  identical treatment (low container defaults are the same hazard). Windows has no
  per-process descriptor limit to raise; `platform.FDLimits{Supported: false}` and a plain
  "not applicable" line in `doctor`'s output is the coherent Windows path (AGENT.md §14: a
  feature working on two of three OSes is not done), not a stub.
...
pointed at an unroutable host) matches every acceptance line above.
