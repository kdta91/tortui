...
// openURL runs `open <rawURL>` — macOS's own URL-open verb (AGENT.md §13).
// rawURL has already been validated as an absolute http(s) URL by
// OpenURL; this never builds a shell command line from it.
func openURL(rawURL string) error {
	return runCommandFunc(exec.Command("open", rawURL))
}
