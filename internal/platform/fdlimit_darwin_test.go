...
func TestRaiseFDLimitRaisesWhenSoftIsBelowHardCeiling(t *testing.T) {
	...
	getrlimitFunc = func(which int, lim *unix.Rlimit) error {
		...
		*lim = unix.Rlimit{Cur: 256, Max: 4096}
		return nil
	}
	...
	if !setCalled {
		t.Fatal("RaiseFDLimit() did not call Setrlimit even though the mocked Cur (256) < Max (4096)")
	}
	...
	if gotLim.Cur != 4096 || gotLim.Max != 4096 {
		t.Fatalf("Setrlimit called with %+v, want Cur=Max=4096", gotLim)
	}

	if limits.Soft != 256 {
		t.Fatalf("RaiseFDLimit().Soft = %d, want the mocked original 256", limits.Soft)
	}

	if limits.Hard != 4096 {
		t.Fatalf("RaiseFDLimit().Hard = %d, want the mocked hard ceiling 4096", limits.Hard)
	}

	if limits.Raised != 4096 {
		t.Fatalf("RaiseFDLimit().Raised = %d, want the mocked raised value 4096", limits.Raised)
	}
}
