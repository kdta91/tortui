...
func TestTryLockFileExclusiveAcrossDescriptors(t *testing.T) {
	...
	if !ok {
		t.Fatal("TryLockFile(f1) = false, want the first descriptor to win the lock")
	}
	...
	if ok2 {
		t.Fatal("TryLockFile(f2) = true, want the second descriptor to be denied while the first holds the lock")
	}
	...
	// A fresh descriptor must be able to acquire the lock now that every
	// descriptor referencing the original open file description is closed.
	...
	if !ok3 {
		t.Fatal("TryLockFile(f3) = false, want the lock to be free after the holder closed its descriptor")
	}
}
